...
echo -e "yCkU588n3pFMYY9*" | docker run -i \
  -v strongswan-data:/data \
  griffinplus/strongswan \
  init \
  --ca-key-type="rsa4096" \
  --server-key-type="rsa3072" \
  --client-key-type="rsa3072" \
  --ca-cert-subject="CN=VPN Root CA,OU=IT Department,O=Example Corp,C=US" \
  --server-cert-subject="CN=vpn.example.com,OU=Network Operations,O=Example Corp,C=US" \
  --client-cert-subject="CN=John Doe,OU=Engineering,O=Example Corp,C=US"
```
